...
root = Tk()
root.title("Ravi Oil Store Entry")
...
def show_accounts(ev):
    ...
    for i in list.curselection():
        ...
        for i in range(5):
            try:
                e=Entry(output_frame,font=("Calibri", 15))
                e.grid(row=i+1, column=1,padx=80,pady=2)
                ...
                f=Entry(output_frame, font=("Calibri", 15))
                f.grid(row=i + 1, column=2, padx=80, pady=2)
                f.bind("<KeyRelease>", calculate)
                f.bind("<Down>", movement_down)
                f.bind("<Up>", movement_up)
                f.bind("<Return>", movement_down)
                f.bind("<Escape>", enter)
                output_frame.pack(padx=2,pady=2)
            except Exception as e:
                print(e)
        f.bind("<Return>", jump_to_save)
...
def movement_down(e):
    ...
    if total_row-1 >= current_row:
        output_frame.grid_slaves(current_row,column=current_column)[0].focus_set()
...
def save_to_main():
    ...
    for i in range(1,6):
        print(i)
        date = output_frame.grid_slaves(row=i,column=1)[0].get()
        amt = output_frame.grid_slaves(row=i,column=2)[0].get()
        if date == "":
            break
        pointer.writerow([date,amt])
    f.close()
    wb.close()
